...
int data_eval::point(size_t nv, const ubvector &pars, double &log_weight,
  ...
  if (pars[pvi["Lambdat"]]<ligo_data.get_grid(2,0) ||
      pars[pvi["Lambdat"]]>ligo_data.get_grid(2,ligo_data.get_size(2)-1)) {
    dat[dvi["ligo_fail"]]=1.0;
    if (verbose>0) {
      cout.precision(10);
  ...
  // --------------------------------------------------------------
  // Interpolate to get individual radii for stars with M-R data
  
  for(size_t i=0;i<nd.list.size();i++) {
    nsmr &c=nd.list[i];
    if (pars[pvi[(std::string("mf_"))+c.name]]>1.0) {
      if (verbose>0) {
        std::cout << "Fail: mass greater than maximum." << std::endl;
      }
      if (err_as_small_wgt) {
        log_weight=-821.0;
        //for(size_t k=0;k<ndat;k++) dat[k]=0.0;
  ...
  // This is now done in nsmr_like.cpp
  //for(size_t i=0;i<nd.list.size();i++) {
  //nsmr &c=nd.list[i];
  //double masst=pars[pvi[(std::string("mf_"))+c.name]]*
  //(dat[dvi["Mns_max"]]-1.0)+1.0;
  //dat[dvi[((std::string)"Rns_"+c.name)]]=tmvsr.interp("gm",masst,"r");
  //}
  
  // This is now done in sxrt_like.cpp
  //for(size_t i=0;i<sd.list.size();i++) {
  //sxrt &c=sd.list[i];
  //double masst=pars[pvi[(std::string("mf_"))+c.name]]*
  //(dat[dvi["Mns_max"]]-1.0)+1.0;
  ...
  if (no_sxrt==false) {
    compute_ss(nv,pars,log_weight,dat);
    copy_ss(dat);
    double lw_sxrt=0.0;
