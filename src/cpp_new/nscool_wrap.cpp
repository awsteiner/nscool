...
void nscool_wrap::hhj_eos(double mass) {
  ...
  for(size_t ix=0;ix<153;ix++) {
    ...
    vector<double> line={((double)ix),r*1.0e3,nc_prof->interp("r",r,"nb"),
    ...
    nscool_tov.line_of_data(line.size(),line);
  }
...
void nscool_wrap::print_temp(int istep, int itprint,
  ...
  if (itprint==0) {
    ...
    tl_prof.line_of_names(((std::string)"rho ephi vol Tinf Linf qnu ")+
			  ...
			  "cv_sm cv_s0 cv_sp cv_q tcn tcp");
...
void nscool_wrap::write_cool_curve(std::string fname) {
  ...
  for(size_t i=0;i<v_time.size();i++) {
    vector<double> line={v_time[i],v_tptr[i],v_lneut[i],v_lphot[i],
		    v_lheat[i]};
