...
data_eval::data_eval() : nw("./") {
  ...
  // Log mdot grid
  o2scl::uniform_grid<double> log_mdot_temp =
      o2scl::uniform_grid_end<double>(-12, -9, 29);
...
void data_eval::init(size_t nparam_, o2scl::vec_index &dvi_,
                     o2scl::vec_index &pvi_, size_t verbose_, ins_data &id_,
...
int data_eval::point(size_t nv, const ubvector &pars, double &log_weight,
  ...
  if (check_timing) {
    ...
    std::cout << "Execution time saturation: " << elapsed.count()
              << " seconds\n";
    // Start nuclei
    start = std::chrono::high_resolution_clock::now();
  }
  ...
  if (!no_nuclei) {
    ...
    L_Pb208 =
        exp(-((rn.etot + 7.87) * (rn.etot + 7.87)) / (2.0 * 0.16 * 0.16)) *
        exp(-((rn.r_charge - 5.50) * (rn.r_charge - 5.50)) /
            (2.0 * 0.12 * 0.12));

    if (!no_prex) {
      L_Pb208 *=
          exp(-((rn.rnrp - 0.283) * (rn.rnrp - 0.283)) / (2.0 * 0.071 * 0.071));
      dat[dvi["lw_prex"]] =
          -((rn.rnrp - 0.283) * (rn.rnrp - 0.283)) / (2.0 * 0.071 * 0.071);
    } else {
      dat[dvi["lw_prex"]] = 0.0;
    }
    dat[dvi["lw_nuc"]] =
        -((rn.etot + 7.87) * (rn.etot + 7.87)) / (2.0 * 0.16 * 0.16) -
        ((rn.r_charge - 5.50) * (rn.r_charge - 5.50)) / (2.0 * 0.12 * 0.12);

    dat[dvi["etot_Pb208"]] = rn.etot;
    dat[dvi["rcharge_Pb208"]] = rn.r_charge;
    dat[dvi["rnrp_Pb208"]] = rn.rnrp;
    if (verbose > 0) {
      std::cout << "Pb 208: " << rn.etot << " " << rn.r_charge << " " << rn.rnrp
                << std::endl;
    }
  ...
  if (!no_nuclei) {
    ...
    L_Zr90 =
        exp(-((rn2.etot + 8.71) * (rn2.etot + 8.71)) / (2.0 * 0.16 * 0.16)) *
        exp(-((rn2.r_charge - 4.27) * (rn2.r_charge - 4.27)) /
            (2.0 * 0.12 * 0.12));
  ...
  if (false) {
    ...
    L_Ca40 = exp(-((rn.etot + 8.45) * (rn.etot + 8.45)) / (2.0 * 0.16 * 0.16)) *
             exp(-((rn.r_charge - 3.48) * (rn.r_charge - 3.48)) /
                 (2.0 * 0.12 * 0.12));
    ...
    if (verbose > 0) {
      std::cout << "Ca 40: " << rn.etot << " " << rn.r_charge << " " << rn.rnrp
                << std::endl;
    }
  ...
  if (verbose > 1 && !no_nuclei) {
    std::cout << "L_Pb208, L_Zr90, L_Ca40: " << L_Pb208 << " " << L_Zr90 << " "
              << L_Ca40 << std::endl;
  }
  ...
  for (size_t i = 0; i < ngrid; i++) {
    ...
    if (false) {
      std::cout << i << " " << nb_grid[i] << " "
                ...
                << te->interp("nb", nb_grid[i], "np") / nb_grid[i] << std::endl;
  ...
  for (size_t k = 0; k < te->get_nlines(); k++) {
    if (!std::isfinite(te->get("ed", k)) || !std::isfinite(te->get("pr", k)) ||
  ...
  for (size_t j = 0; j < tmvsr.get_nlines(); j++) {
    if (!std::isfinite(tmvsr.get("gm", j)))
      mvsr_finite = false;
  }
  ...
  if (tmvsr.max("gm") < 2.0) {
    if (verbose > 0) {
      std::cout << "Fail: Maximum mass (" << tmvsr.max("gm") << ") too small."
                << std::endl;
    }
    if (err_as_small_wgt) {
      log_weight = -801.0;
  ...
  if (tmvsr.is_column("cs2"))
    tmvsr.delete_column("cs2");

  if (tmvsr.get("ed", 0) == tmvsr.get("ed", tmvsr.get_nlines() - 1)) {
    dat[dvi["eos_fail"]] = 1.0;
    std::cout << "Fail: M-R table problem with ed " << tmvsr.get("ed", 0)
              << std::endl;
    if (err_as_small_wgt) {
      log_weight = -822.0;
      // for(size_t k=0;k<ndat;k++) dat[k]=0.0;
  ...
  if (tmvsr.get("pr", 0) == tmvsr.get("pr", tmvsr.get_nlines() - 1)) {
    dat[dvi["tov_fail"]] = 1.0;
    std::cout << "Fail: M-R table problem with pr " << tmvsr.get("pr", 0)
              << std::endl;
    if (err_as_small_wgt) {
      log_weight = -823.0;
      // for(size_t k=0;k<ndat;k++) dat[k]=0.0;
  ...
  if (te->get("nb", 0) == te->get("nb", te->get_nlines() - 1)) {
    dat[dvi["eos_fail"]] = 1.0;
    std::cout << "Fail: EOS table problem with nb " << te->get("nb", 0)
              << std::endl;
    if (err_as_small_wgt) {
      log_weight = -824.0;
      // for(size_t k=0;k<ndat;k++) dat[k]=0.0;
  ...
  if (te->get("kfn", 0) == te->get("kfn", te->get_nlines() - 1)) {
    dat[dvi["eos_fail"]] = 1.0;
    std::cout << "Fail: EOS table problem with kfn " << te->get("kfn", 0)
              << std::endl;
    if (err_as_small_wgt) {
      log_weight = -825.0;
      // for(size_t k=0;k<ndat;k++) dat[k]=0.0;
  ...
  if (te->get("kfp", 0) == te->get("kfp", te->get_nlines() - 1)) {
    dat[dvi["eos_fail"]] = 1.0;
    std::cout << "Fail: EOS table problem with kfp " << te->get("kfp", 0)
              << std::endl;
    if (err_as_small_wgt) {
      log_weight = -826.0;
      // for(size_t k=0;k<ndat;k++) dat[k]=0.0;
  ...
  if (!no_ligo) {
    ...
    if (pars[pvi["M_chirp"]] < ligo_data.get_grid(0, 0) ||
        pars[pvi["M_chirp"]] >
            ligo_data.get_grid(0, ligo_data.get_size(0) - 1)) {
    ...
    double Lambdat = 16.0 / 13.0 *
                     ((m1 + 12.0 * m2) * pow(m1, 4.0) * Lambda1 +
    ...
    double lw_ligo =
        ligo_data.interp_linear(pars[pvi["M_chirp"]], pars[pvi["q"]], Lambdat);
    log_weight += lw_ligo;
    dat[dvi["Lambdat"]] = Lambdat;
    dat[dvi["lw_ligo"]] = lw_ligo;

    if (verbose > 0) {
      cout << "log weight from LIGO, log weight after: " << lw_ligo << " "
           << log_weight << endl;
    }
  } else {
    double lw_ligo = 0.0;
    ...
    if (true) {
      if (nw.n3_kf < kfn_min || nw.n3_kf > kfn_max) {
        if (verbose > 0) {
          ...
          std::cout << "kfn_min, kfn, kfn_max: " << kfn_min << " " << nw.n3_kf
                    << " " << kfn_max << std::endl;
        }
        if (err_as_small_wgt) {
          log_weight = -816.0;
      ...
      if (nw.p1_kf < kfp_min || nw.p1_kf > kfp_max) {
        if (verbose > 0) {
          ...
          std::cout << "kfp_min, kfp, kfp_max: " << kfp_min << " " << nw.p1_kf
                    << " " << kfp_max << std::endl;
        }
        if (err_as_small_wgt) {
          log_weight = -817.0;
      ...
      if (nw.n3_dk > kfn_max || nw.p1_dk > kfp_max) {
        std::cout << "Delta k_F out of bounds. " << nw.n3_dk << " " << kfn_max
                  << " " << nw.p1_dk << " " << kfp_max << std::endl;
        if (err_as_small_wgt) {
          log_weight = -818.0;
          // for(size_t k=0;k<ndat;k++) dat[k]=0.0;
          return 0;
  ...
  if (check_timing) {
    ...
    std::chrono::duration<double> elapsed = end - start;
    // Output the elapsed time in seconds
    std::cout << "Execution time Fermi mom and radius: " << elapsed.count()
              << " seconds\n";
    start = std::chrono::high_resolution_clock::now();
  }
  ...
  for (int i = te->get_nlines() - 1; i > 0; i--) {
    ...
    nw.nscool_core.set("Rho", temp_index,
                       o2scl::o2scl_settings.get_convert_units().convert(
                           "Msun/km^3", "g/cm^3", ed));
    nw.nscool_core.set("Press", temp_index,
                       o2scl::o2scl_settings.get_convert_units().convert(
  ...
  if (true) {
    for (size_t j = 0; j < nw.nscool_crust.get_nlines(); j++) {
      ...
      nw.nscool_crust.set("rho", j,
                          o2scl::o2scl_settings.get_convert_units().convert(
                              "Msun/km^3", "g/cm^3", ed));
      nw.nscool_crust.set("P", j,
                          o2scl::o2scl_settings.get_convert_units().convert(
                              "Msun/km^3", "dyne/cm^2", pr));
  ...
  if (check_timing) {
    ...
    std::chrono::duration<double> elapsed = end - start;
    // Output the elapsed time in seconds
    std::cout << "Execution time set-Nscool: " << elapsed.count()
              << " seconds\n";
    start = std::chrono::high_resolution_clock::now();
  }
  ...
  if (!no_ins && !no_sxrt) {
    ...
    if (!flag_emu_aws) {
      ...
      for (size_t imass = 0; imass < 6; imass++) {
        ...
        // ---------------------------------------------------------
        // Compute the neutron star structure and store the EOS in a table for
        // the NScool object
        ...
        if (f_ret != 0) {
          if (verbose > 0) {
            std::cout << "Fail: Object nstar_cold failed to "
                      << "compute profile of a " << ns_mass << " neutron star."
                      << std::endl;
          }
          log_weight = -846.0;
          return 0;
        ...
        // Note that this is different from class variable 'tmvsr', which stores
        // the full M-R curve.
        ...
        for (size_t i = 0; i < tov_tab->get_nlines(); i++) {
          ...
          nw.nscool_tov.set("density", i,
                            tov_tab->get("ed", i) * 3.5176758e+14);
          nw.nscool_tov.set("pressure", i,
                            tov_tab->get("pr", i) * 3.1615293e+34);
        ...
        for (size_t ieta = 0; ieta < 3; ieta++) {
          ...
          // Control the number of timesteps between output. A large number
          // effectively specifies no output.
          if (verbose > 1) {
            nw.main_out_it = 20;
          } else {
            nw.main_out_it = 500;
          ...
          double T_surf =
              (ephi_surf * nw.tl_prof.get(r_grid.size() - 1, j, "Tinf")) / 1e8;
          int_cv /= T_surf;
          ...
          for (size_t k = 0; k < 30; k++) {
            ...
            int iret2 = acc_compute(logT, Q_heat, ns_mass, R, Mdot, lphot,
                                    lneut, lheat);

            if (verbose > 1) {
              cout << nw.eta << " " << ns_mass << " " << Mdot << " " << logT
                   << " " << lphot << endl;
            }
  ...
  if (check_timing) {
    ...
    std::chrono::duration<double> elapsed = end - start;
    // Output the elapsed time in seconds
    std::cout << "Execution time INS and SXRT: " << elapsed.count()
              << " seconds\n";
  }
  ...
  if (false) {
    ...
    if (log_weight > -799.0) {
      ...
      std::cout << "intermp_idw estimate: " << vout[0] << " " << verr[0] << " "
                << exp(log_weight) << std::endl;
    }
  }
  ...
  if (check_timing) {
    ...
    // Output the elapsed time in seconds
    std::cout << "Execution time point function: " << elapsed.count()
              << " seconds\n";
  }
