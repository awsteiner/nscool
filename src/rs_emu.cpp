...
void emulator_rs::train(o2scl::vec_index &pvii, o2scl::vec_index &dvii,
                        data_eval *depi) {
  ...
  std::vector<std::string> list_in = {
      "log10_Tcn", "k_maxn", "del_kn", "log10_Tcp", "k_maxp", "del_kp", "b",
      "c",         "c_r",    "m_s",    "zeta",      "c_s",    "c_w",    "b1",
      "xi",        "a1",     "a2",     "a3",        "a4",     "a5",     "a6",
      "b2",        "b3",     "alpha",  "Q"};
  ...
  for (size_t ieta = 0; ieta < 3; ieta++) {
    for (size_t imass = 0; imass < 6; imass++) {
      for (size_t i = 0; i < ngrid; i++) {
        ins_out.push_back(((std::string) "cL_") + o2scl::szttos(ieta) + "_" +
  ...
  for (size_t ieta = 0; ieta < 3; ieta++) {
    for (size_t imass = 0; imass < 6; imass++) {
      for (size_t i = 0; i < 30; i++) {
        sxrt_out.push_back(((std::string) "sL_") + o2scl::szttos(ieta) + "_" +
                           o2scl::szttos(imass) + "_" + o2scl::szttos(i));
      }
    }
  }
  ...
  for (size_t k = 0; k < files.size(); k++) {
    ...
    std::cout << "Rank " << mpi_rank << " reading training table "
              << "with index " << k << " from file " << files[k] << std::endl;

    hdf_file hf;
    hf.open(files[k]);
    hdf_input(hf, tab_k);
    hf.close();
    std::cout << "Rank " << mpi_rank << ": table has " << tab_k.get_nlines()
              << " lines." << std::endl;
  ...
  ip_1->set_data_tensor(in_size[1], out_size[1], tab_k.get_nlines(), tin, tout);
  ip_2->set_data_tensor(in_size[1], out_size2[1], tab_k.get_nlines(), tin,
                        tout2);
...
mcmc_wrapper::mcmc_wrapper() {
  ...
  std::vector<std::string> temp_names_units = {
      "log10_Tcn", "K",       "k_maxn", "1/fm",   "del_kn", "1/fm", "log10_Tcp",
      "K",         "k_maxp",  "1/fm",   "del_kp", "1/fm",   "b",    "",
      "c",         "",        "c_r",    "fm",     "m_s",    "1/fm", "zeta",
      "",          "c_s",     "fm",     "c_w",    "fm",     "b1",   "",
      "xi",        "",        "a1",     "1/fm",   "a2",     "",     "a3",
      "fm",        "a4",      "fm^2",   "a5",     "fm^3",   "a6",   "fm^4",
      "b2",        "fm^2",    "b3",     "fm^4",   "alpha",  "1/fm", "Q",
      "MeV",       "M_chirp", "Msun",   "q",      ""};
...
int mcmc_wrapper::steady_state(std::vector<std::string> &sv, bool itive_com) {
  ...
  for (double Mdot = 6.0e-12; Mdot < 1.0e-8; Mdot *= 1.2) {
    ...
    int iret =
        de[0].acc_compute(logT, Q_heat, mass, rad, Mdot, lphot, lneut, lheat);
    if (iret != 0) {
...
int mcmc_wrapper::clean(std::vector<std::string> &sv, bool itive_com) {
  ...
  for (size_t i = 0; i < t.get_nlines(); i++) {
    if (i % 500 == 499) {
      std::cout << "Progress: i+1= " << i + 1 << " of " << t.get_nlines()
                << endl;
    }
    ...
    for (size_t j = 0; j < nparam && out_of_bounds == false; j++) {
      if (t.get(j + 5, i) < low[j]) {
        ...
        cout << low[j] << " " << t.get(j + 5, i) << " " << high[j] << endl;
        out_of_bounds = true;
        list.push_back(i);
      }
      ...
      for (size_t j = i + 1; j < t.get_nlines(); j++) {
        ...
        if (i < t.get_nlines() && j < t.get_nlines() && j > i) {
          for (size_t k = 5; k < nparam + 5 && match == true; k++) {
            if (fabs(t.get(k, i)) > tol_abs || fabs(t.get(k, j)) > tol_abs) {
              if (fabs(t.get(k, i) - t.get(k, j)) /
                      fabs(t.get(k, i) + t.get(k, j)) >
                  tol_rel) {
                match = false;
              }
            }
        ...
        if (match == true) {
          list.push_back(j);
          std::cout << "Match between rows " << i << " and " << j << std::endl;
  ...
  if (sv.size() > 2) {
    ...
    hf.close();
    std::cout << "Wrote " << t.get_nlines() << " lines to file " << sv[2]
              << std::endl;
  }
...
int mcmc_wrapper::mcmc(std::vector<std::string> &sv, bool itive_com) {
  ...
  if (((int)de_size) < n_threads) {
    cout << "Creating de object for " << n_threads << " threads in mcmc()."
         << endl;
    if (de_size > 0)
      delete de;
    de = new data_eval[n_threads];
      ...
      if (c.mdot_ul) {
        ...
        step[pvi[(string("log10_mdot_")) + c.name]] =
            (high[pvi[(string("log10_mdot_")) + c.name]] -
             low[pvi[(string("log10_mdot_")) + c.name]]) /
            20.0;
      }
  ...
  if (!jan_emu) {
    ...
    vector<string> temp_names_units2 = {"etot_Pb208",
                                        "MeV",
                                        "rcharge_Pb208",
                                        "fm",
                                        "rnrp_Pb208",
    ...
    cout << "X1: " << pvi.size() << " " << pnames.size() << " " << punits.size()
         << " " << nparam << endl;
    ...
    for (size_t ieta = 0; ieta < 3; ieta++) {
      for (size_t imass = 0; imass < 6; imass++) {
        for (size_t i = 0; i < ngrid; i++) {
          dnames.push_back(((std::string) "cT_") + o2scl::szttos(ieta) + "_" +
                           o2scl::szttos(imass) + "_" + o2scl::szttos(i));
          dunits.push_back("K");
        }
      }
    ...
    for (size_t ieta = 0; ieta < 3; ieta++) {
      for (size_t imass = 0; imass < 6; imass++) {
        for (size_t i = 0; i < ngrid; i++) {
          dnames.push_back(((std::string) "cL_") + o2scl::szttos(ieta) + "_" +
                           o2scl::szttos(imass) + "_" + o2scl::szttos(i));
    ...
    for (size_t ieta = 0; ieta < 3; ieta++) {
      for (size_t imass = 0; imass < 6; imass++) {
        for (size_t i = 0; i < 30; i++) {
          dnames.push_back(((std::string) "sL_") + o2scl::szttos(ieta) + "_" +
                           o2scl::szttos(imass) + "_" + o2scl::szttos(i));
          dunits.push_back("erg/s");
        }
      }
    ...
    */
    ...
    vector<string> temp_names_units2 = {"emulated", "", "log_wgt_unc", ""};
    // Then continue with the output names and units
    for (size_t i = 0; i < temp_names_units2.size(); i += 2) {
      dnames.push_back(temp_names_units2[i]);
      dunits.push_back(temp_names_units2[i + 1]);
  ...
  if (pnames.size() != nparam || punits.size() != nparam) {
  ...
  if (dnames.size() != ndat || dunits.size() != ndat) {
  ...
  if (false) {
    ...
    for (size_t jj = 0; jj < 10000; jj++) {
      for (size_t i = 0; i < g1.get_nlines(); i++) {
        ...
        cout << "row,log_wgt: " << row << " " << g2.get("log_wgt", row) << endl;
        ...
        */
        ...
        if (true) {
          for (size_t j = 11; j <= 27; j++) {
            if (j < 19) {
              cout << out.get_column_name(j) << " "
                   << g2.get_column_name(j + 29) << " " << g2.get(j + 29, row)
                   << endl;
              pars[j - 5] = g2.get(j + 29, row);
            } else {
              cout << out.get_column_name(j) << " "
                   << g2.get_column_name(j + 45) << " " << g2.get(j + 45, row)
                   << endl;
        ...
        cout << "Rank " << mpi_rank << " done: " << ret << " " << pars[0] << " "
             << log_wgt << endl;
        ...
        if (ret == 0) {
          ...
          for (size_t k = 0; k < nparam; k++)
            line.push_back(pars[k]);
          for (size_t k = 0; k < ndat; k++)
            line.push_back(dat[k]);
  ...
  if (ptype == ((std::string) "step")) {
    ...
    if (false) {
      ...
      for (size_t i = 0; i < nparam; i++) {
        ...
        cout << pnames[i] << " ";
        cout.width(4);
        cout << punits[i] << " " << low[i] << " " << high[i] << " " << step[i]
             << endl;
      }
    ...
    cout << "Rank " << mpi_rank << " reading file " << fnamex << "." << endl;
    ...
    for (size_t j = 0; j < nparam; j++) {
      avg[j] = wvector_mean(tprev.get_nlines(), tprev[j + 5], tprev["wgt"]);
      std[j] = wvector_stddev(tprev.get_nlines(), tprev[j + 5], tprev["wgt"]);
      cout << j << " " << pvi[j] << " " << avg[j] << " " << std[j] << endl;
    }
  ...
  if (false) {
    ...
    for (size_t i = 0; i < nparam; i++) {
      cout.setf(ios::showpos);
      cout.width(15);
      cout << pvi[i] << " " << init[i] << " " << low[i] << " " << high[i]
           << endl;
      cout.unsetf(ios::showpos);
    }
    exit(-1);
  ...
  cout << "Going to mcmc_fill() " << nparam << " " << low.size() << " "
       << high.size() << " " << vpf.size() << " " << vff.size() << " "
       << vdat.size() << " " << ndat << endl;
...
int mcmc_wrapper::initial_point_last(std::vector<std::string> &sv,
  ...
  size_t offset = 5;
  if (sv.size() >= 3)
    offset = o2scl::stoszt(sv[2]);

  cout << "Rank " << mpi_rank << " loading last initial points from file '"
       << fname << "' with offset " << offset << "." << endl;
...
int mcmc_wrapper::initial_point_first(std::vector<std::string> &sv,
  ...
  cout << "Rank " << mpi_rank << " loading first point from file '" << fname
       << "'." << endl;
  ...
  int row = 0;
  if (verbose > 0) {
    std::cout << "Initial point at row " << row
              << " has log_weight= " << tip.get("log_wgt", row) << std::endl;
  }
...
int mcmc_wrapper::initial_point_best(std::vector<std::string> &sv,
  ...
  double prec = 1.0e-6;
  if (sv.size() >= 3)
    prec = o2scl::stod(sv[2]);
  size_t offset = 6;
  if (sv.size() >= 4)
    offset = o2scl::stoszt(sv[3]);

  cout << "Rank " << mpi_rank << " loading best initial points from file '"
       << fname << "'." << endl;
  cout << "Number of parameters is " << nparam << " precision is " << prec
       << " and offset is " << offset << endl;
...
int mcmc_wrapper::initial_point_rand(std::vector<std::string> &sv,
  ...
  cout << "Rank " << mpi_rank << " loading random initial point from file '"
       << fname << "'." << endl;
  ...
  if (mct.verbose > 0) {
    std::cout << "Initial points: Finding " << n_points
              << " random points from file named " << fname << " ."
              << std::endl;
  }
  ...
  for (size_t k = 0; k < n_points; k++) {
    if (mct.verbose > 0) {
      std::cout << "Initial point " << k << " at row " << rows[k]
                << " has log_weight= " << tip.get("log_wgt", rows[k])
                << std::endl;
    }
...
int mcmc_wrapper::make_gaussian(std::vector<std::string> &sv, bool itive_com) {
  ...
  for (size_t i = 0; i < nparam; i++) {
    ...
    if (weighted) {
      peak[i] =
          o2scl::wvector_mean<mat_col_t>(t.get_nlines(), col, t["weight"]);
      // Square standard deviation
      covar(i, i) =
          o2scl::wvector_stddev<mat_col_t>(t.get_nlines(), col, t["weight"]);
  ...
  */
  ...
  for (size_t i = 0; i < nparam; i++) {
    ...
    cout << "Parameter " << i << " " << t.get_column_name(i + 5)
         << " min: " << minv[i] << " max: " << maxv[i] << " peak: " << peak[i]
         << " std: " << std << endl;
    covar(i, i) = std * std;
  }
...
double
mcmc_wrapper::gauss_min_func(size_t np, const ubvector &p,
...
double
mcmc_wrapper::gauss_min_func2(size_t np, const ubvector &p,
                              table_units<> &prev_res,
                              o2scl::prob_dens_mdim_gaussian<> &mdim_gauss) {
  ...
  for (size_t row = 0; row < prev_res.get_nlines(); row++) {
    ...
    double exact = prev_res.get("log_wgt", row);
    if (row % 40 == 0) {
      cout << row << " " << mdim_gauss.pdf(xv) << " " << mdim_gauss.log_pdf(xv)
           << " " << model << " " << exact << endl;
    }
    ret += pow((model - exact) / fabs(exact) * 100.0, 2.0);
  }
...
int mcmc_wrapper::fit_gaussian(std::vector<std::string> &sv, bool itive_com) {
  ...
  multi_funct mf =
      std::bind(std::mem_fn<double(size_t, const ubvector &, table_units<> &,
                                   o2scl::prob_dens_mdim_gaussian<> &)>(
                    &mcmc_wrapper::gauss_min_func),
                this, std::placeholders::_1, std::placeholders::_2, std::ref(t),
                std::ref(pdmg2));
...
int mcmc_wrapper::check_limits(std::vector<std::string> &sv, bool itive_com) {
  ...
  for (size_t i = 0; i < n_params; i++) {
    ...
    cout.precision(4);
    cout.setf(ios::showpos);
    cout << low2[i] << " " << min << " " << mean - sd_loc << " " << mean << " "
         << mean + sd_loc << " " << max << " " << high2[i];
    if (mean + sd_loc > high2[i])
      cout << " x";
    if (mean - sd_loc < low2[i])
      cout << " y";
    cout << endl;
    cout.precision(6);
    cout.unsetf(ios::showpos);
